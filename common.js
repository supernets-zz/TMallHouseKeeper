...
common.appName = "TMallHouseKeeper";
common.destAppName = "手机天猫";
common.destPackageName = "com.tmall.wireless";

//Min(下一次充值中心签到奖励检查时间戳, 下一次充值中心收饲料检查时间戳, 下一次充值中心免费扭蛋检查时间戳, 下一个整点检查时间戳)
common.nextChargeCenterSignInBonusTimestampTag = "下一次充值中心签到奖励检查时间戳";  //带毫秒
common.nextChargeCenterStealFeedTimestampTag = "下一次充值中心收饲料检查时间戳";  //带毫秒
common.nextChargeCenterFreeLuckyEggTimestampTag = "下一次充值中心免费扭蛋检查时间戳";  //带毫秒
...
common.queryList = function (json, depth, arr) {
    log(json.className() + "(" + depth + "): " + json.childCount());
    for (var i = 0; i < json.childCount(); i++) {
        var sonList = json.child(i);
        if (sonList.childCount() == 0) {
            arr.push(json.child(i));
            log(json.className() + "(" + depth + ").child(" + i + "): " + json.child(i));
        } else {
            if (depth > 0) {
                log(json.className() + "(" + depth + ")[" + i + "]: " + sonList);
...
common.waitImageDismiss = function (tmpl, sec) {
    var point = null;
    var startTick = new Date().getTime();
    for (;;) {
        point = common.findImage(tmpl);
