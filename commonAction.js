...
judgeAppMainPage = function () {
    ...
    var selectedTab = packageName(common.destPackageName).desc("精选").visibleToUser(true).findOne(1000);
    if (selectedTab == null) {
        log("judgeAppMainPage: 精选 not exist");
        return false;
    }

    var categoryTab = packageName(common.destPackageName).desc("分类").visibleToUser(true).findOne(1000);
    if (categoryTab == null) {
        log("judgeAppMainPage: 分类 not exist");
        return false;
    }

    var playTab = packageName(common.destPackageName).desc("拍照").visibleToUser(true).findOne(1000);
    if (playTab == null) {
        log("judgeAppMainPage: 拍照 not exist");
        return false;
    }

    var cartTab = packageName(common.destPackageName).desc("购物车").visibleToUser(true).findOne(1000);
    if (cartTab == null) {
        log("judgeAppMainPage: 购物车 not exist");
        return false;
    }

    var mineTab = packageName(common.destPackageName).desc("我").visibleToUser(true).findOne(1000);
    if (mineTab == null) {
        log("judgeAppMainPage: 我 not exist");
        return false;
    }
